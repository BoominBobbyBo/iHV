...
#    - Processes "Saves\Person","Saves\scene", "Saves\Pose"
...
Function Update-Files {
    ...
    Param (
        ...
        $Content = [System.IO.File]::ReadAllLines($File_FullName) # .NET function (requires .NET) (Get-Content -raw $File_FullName)
        $blnWasTheFileChanged = $false

        # Global Changes
        If($Content -match "SELF:/"){$blnWasTheFileChanged = $true}
        $Content = $Content -Replace("SELF:/","")

        # RESOURCE PATHS
        # Update $Content\resource paths:
        $Content | Where-Object {$_ -match "/" -Or $_.indexOf("presetName") -ge 0} | ForEach-Object {
            $Line = $_ 
            #Write-Debug $Line
            ...
            # ---------------------------> Remove VAR Paths
                ...
                If($NodeName.indexOf("presetName") -ge 0 ){
                    ...
                    If($NodeValue.indexOf(":") -gt 0){
                        ...
                        $Content = $Content -ireplace $NodeValue, ( $NodeValue.Replace($RemoveMe, "") )
                    }
                } 
                ...
                if($NodeValue -match "/Custom"){
                    ...
                    $Content = $Content -ireplace [regex]::Escape($Line), $NewLine
        ...
        # Final global fixes
        $Content = $Content -Replace("ustom/","Custom/") -Replace("cCustom/","Custom/") -Replace("CCustom/","Custom/")

        If( ($blnWasTheFileChanged -eq $true)){
            Write-Host $ScriptName FILE Updated: $File_FullName
            try{ [System.IO.File]::WriteAllLines($File_FullName, $Content) }
