...
If($blnLaunched -ne $true){ 
    Read-Host -Prompt 'You must run NormalizeVAM4VR.ps1 on extracted content first (hit ENTER to continue)' 
}
...
$blnCleanTextures      = $false # Normalize texture files into the parent texture folder. Will impact game quality but increase stability.
$blnCleanIdiotContent  = $true # Expand the Idiots array to known offenders. Keep on when dealing with new content. Turn off when processing mature installs.
...
If($blnCleanTextures = $false){ $Exceptions += "Custom/Atom/Person/Textures"}
...
$allResourcePaths | Foreach-Object { 
  ...
  Get-ChildItem -Path ($vamRoot + $ResourceDir) -Directory -Force -ErrorAction SilentlyContinue | Foreach-Object { # if this faults, directory in the array doesn't exist in the file system
    ...
    If($blnException -eq $true){ $LogEntry + "----IDIO EXCEPTION: " + $_.FullName | Out-File -FilePath $LogPath -Append} 
    ElseIf($blnIsRootFolder -eq $true) { $LogEntry + "----IDIO EXCEPTION: " + $_.FullName | Out-File -FilePath $LogPath -Append}
...
Get-ChildItem -Path ($vamRoot + "Custom") -File -Recurse -Force | Where-Object { $_.Name -ilike "*.var" -or $_.Name -ilike "*.zip" -or $_.Name -ilike "*.rar" -or $_.Name -ilike "meta.json" -or $_.Name -ilike "*.txt" -or $_.Name -ilike "*.dds" -or $_.Name -ilike "*.obj" -or $_.Name -ilike "*.dds" -or $_.Name -ilike "*.duf" -or $_.Name -ilike "*.dsf" -or $_.Name -ilike "*.psd" -or $_.Name -ilike "*.obj" -or $_.Name -ilike "*.info" -or $_.Name -ilike "*.meta"} | Remove-Item -Force
Get-ChildItem -Path ($vamRoot + "Saves") -File -Recurse -Force | Where-Object { $_.Name -ilike "*.var" -or $_.Name -ilike "*.zip" -or $_.Name -ilike "*.rar" -or $_.Name -ilike "meta.json" -or $_.Name -ilike "*.txt" -or $_.Name -ilike "*.dds" -or $_.Name -ilike "*.obj" -or $_.Name -ilike "*.dds" -or $_.Name -ilike "*.duf" -or $_.Name -ilike "*.dsf" -or $_.Name -ilike "*.psd" -or $_.Name -ilike "*.obj" -or $_.Name -ilike "*.info" -or $_.Name -ilike "*.meta"} | Remove-Item -Force
